...
describe("predictFinish", () => {
  ...
  const midBike = athlete("1", {
    swimF: START + 84 * MIN, bikeS: START + 92 * MIN, sumiyoshi: START + 4 * HOUR + 12 * MIN,
  });
  ...
  it("never predicts a slower finish for a faster athlete at the same point", () => {
    const fast = athlete("f", {
      swimF: START + 70 * MIN, bikeS: START + 76 * MIN, sumiyoshi: START + 3 * HOUR + 20 * MIN,
    });
    const slow = athlete("s", {
      swimF: START + 100 * MIN, bikeS: START + 110 * MIN, sumiyoshi: START + 5 * HOUR + 30 * MIN,
    });
  ...
  it("predicts a plausible B finish using the swim as a percentile", () => {
    ...
    const b = athlete("3001", {
      swimF: bStart + 25 * MIN, bikeS: bStart + 33 * MIN, sumiyoshi: bStart + 78 * MIN,
