...
/** Tabs are controlled, so the test owns the selection state. */
...
describe("Button", () => {
  ...
  it("keeps the brand gradient on the default variant", () => {
    render(<Button>追加</Button>);
    expect(screen.getByRole("button", { name: "追加" })).toHaveClass("bg-brand-gradient");
  });

  it("lets the caller override the background", () => {
    render(<Button className="bg-muted">追加</Button>);
    const button = screen.getByRole("button", { name: "追加" });
...
describe("Footer", () => {
  it("credits AI TRI+ with an exact outbound link", () => {
    render(<Footer />);
  ...
  it("credits the timing provider", () => {
    render(<Footer />);
